...
export default function AdminVenues() {
    ...
    const openEdit = (v: any) => {
        ...
        setForm({
            ...
            lat: String(v.location?.coordinates?.lat || ''),
            lng: String(v.location?.coordinates?.lng || ''),
            pricePerHour: String(v.pricePerHour),
            description: v.description || '',
            sports: v.sports?.map((s: any) => s.name).join(', ') || '',
    ...
    const handleSave = async () => {
        if (!form.name || !form['location.address'] || !form.pricePerHour) {
            Alert.alert('Required', 'Name, address and price are required');
            return;
        }
        setSaving(true);
        try {
            ...
            const payload = {
                ...
                sports: form.sports.split(',').map((s: string) => ({ name: s.trim() })).filter((s: any) => s.name),
                amenities: form.amenities.split(',').map((s: string) => s.trim()).filter(Boolean),
            };
            const r = editId
                ? await adminApi.updateVenue(editId, payload)
                : await adminApi.createVenue(payload);
            if (r.success) {
                setShowModal(false);
                load();
                Alert.alert('✅ Success', `Venue ${editId ? 'updated' : 'created'} successfully!`);
            } else {
                Alert.alert('Error', r.message || 'Failed to save venue.');
            }
        } finally {
            setSaving(false);
        }
            ...
            <View style={styles.cardRow}>
                <View style={styles.cardInfo}>
                    ...
                    {item.location?.coordinates?.lat ? (
                        ...
                        <Text style={styles.venueCoordsEmpty}>📍 No coordinates</Text>
                    )}
                        ...
                        >
                            ...
                            <View style={styles.fieldGroup}>
                                ...
                                <TextInput
                                    ...
                                    onChangeText={(v) => setForm((p: any) => ({ ...p, 'location.address': v }))}
                                    placeholder="Address *"
                                    placeholderTextColor="#bbb"
                                />
                            </View>
                            ...
                            <View style={styles.coordsCard}>
                                <Text style={styles.coordsTitle}>📍 Map Location (optional)</Text>
                                <Text style={styles.coordsHint}>
                                    Enter latitude & longitude so the map shows the exact turf pin.
                                ...
                                <View style={styles.coordsRow}>
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <TextInput
                                            style={styles.input}
                                            value={form.lat}
                                            onChangeText={(v) => setForm((p: any) => ({ ...p, lat: v }))}
                                    ...
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <TextInput
                                            style={styles.input}
                                            value={form.lng}
                                            onChangeText={(v) => setForm((p: any) => ({ ...p, lng: v }))}
                            ...
                            <View style={styles.fieldGroup}>
                                ...
                                <TextInput
                                    ...
                                    onChangeText={(v) => setForm((p: any) => ({ ...p, pricePerHour: v }))}
                            ...
                            <View style={styles.fieldGroup}>
                                ...
                                <TextInput
                                    style={styles.input}
                                    value={form.sports}
                                    onChangeText={(v) => setForm((p: any) => ({ ...p, sports: v }))}
                            ...
                            <View style={styles.fieldGroup}>
                                ...
                                <TextInput
                                    ...
                                    onChangeText={(v) => setForm((p: any) => ({ ...p, amenities: v }))}
                                    placeholder="e.g. Parking, Lights, Changing Room"
                                    placeholderTextColor="#bbb"
                                />
                            </View>
...
const styles = StyleSheet.create({
    ...
    venueCoords: { fontSize: 11, color: '#10b981', marginBottom: 2 },
    venueCoordsEmpty: { fontSize: 11, color: '#ccc', marginBottom: 2 },
    coordsCard: {
        backgroundColor: '#f0f9ff', borderRadius: 10, padding: 14, gap: 10,
        borderWidth: 1, borderColor: '#bae6fd',
    },
    coordsTitle: { fontSize: 14, fontWeight: '700', color: '#0369a1' },
    coordsHint: { fontSize: 12, color: '#0284c7', lineHeight: 18 },
